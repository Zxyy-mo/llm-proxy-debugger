...
const props = withDefaults(defineProps<{ active?: boolean; selectedTrace?: string | null }>(), { active: true, selectedTrace: null })
const emit = defineEmits<{ select: [log: RequestLog] }>()
const query = ref('')
const status = ref('')
const applied = ref({ query: '', status: '' })
...
async function load(next = '', useApplied = false) {
  controller?.abort()
  const request = controller = new AbortController()
  busy.value = true; error.value = ''
  const filters = useApplied ? { ...applied.value } : { query: query.value, status: status.value }
  try {
    const data = await requestJSON<HistoryResult>(`/api/history?${new URLSearchParams({ q: filters.query, status: filters.status, limit: '50', cursor: next })}`, 'GET', undefined, request.signal)
    if (!request.signal.aborted) { result.value = data; cursor.value = next; applied.value = filters }
...
watch(() => props.active, active => {
  if (active && !result.value) void load()
  if (!active) { controller?.abort(); busy.value = false }
}, { immediate: true })
onUnmounted(() => controller?.abort())
</script>

<template>
  <section class="min-w-0 space-y-4" aria-label="历史查询与清理" :aria-busy="busy">
    <div class="flex flex-wrap items-center gap-2"><h2 class="text-sm font-semibold">历史记录</h2><span v-if="result" class="text-xs text-muted-foreground">{{ result.total.toLocaleString() }} 条 · {{ result.storage.enabled ? 'SQLite 持久化' : '当前仅保存在内存' }}</span><Button size="sm" variant="outline" class="ml-auto" :disabled="busy" @click="load(cursor, true)">刷新</Button></div>
    ...
    <p v-if="busy && !result" role="status" class="p-4 text-xs text-muted-foreground">正在查询历史记录…</p>
    <div class="divide-y rounded-lg border">
      <p v-if="result && !result.items.length" class="p-6 text-center text-xs text-muted-foreground">{{ applied.query || applied.status ? '没有匹配的历史记录。' : '还没有记录，先通过网关发送一次模型请求。' }}<Button v-if="applied.query || applied.status" size="sm" variant="link" :disabled="busy" @click="query = ''; status = ''; load()">清除历史筛选</Button></p>
      <article v-for="log in result?.items ?? []" :key="log.trace_id" class="flex min-w-0 flex-wrap items-center gap-3 p-3" :class="selectedTrace === log.trace_id ? 'bg-primary/5' : ''" :aria-current="selectedTrace === log.trace_id ? 'true' : undefined">
    ...
    <div class="flex flex-wrap items-center gap-2"><span v-if="result?.total" class="text-[11px] text-muted-foreground">第 {{ Number(cursor || 0) + 1 }}–{{ Number(cursor || 0) + result.items.length }} 条</span><Button v-if="cursor" size="sm" variant="outline" :disabled="busy" @click="load('', true)">返回第一页</Button><Button v-if="result?.next_cursor" size="sm" variant="outline" :disabled="busy" @click="load(result.next_cursor, true)">下一页</Button></div>
