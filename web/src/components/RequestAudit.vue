...
import type { AuditMode } from '@/lib/investigation'
import type { ReplayOperationState } from '@/lib/replayOperation'
...
const props = defineProps<{ traceId: string | null; refreshKey: number; replayRefreshKey: number; logs: RequestLog[]; active: boolean }>()
...
const mode = defineModel<AuditMode>('mode', { default: 'compare' })
const chosenSource = ref<ReplaySource | null>(null)
const replayWorkbench = ref<InstanceType<typeof ReplayWorkbench> | null>(null)
...
async function loadCapture() {
  ...
  if (!trace || !props.active) { loading.value = false; return }
  loading.value = true
  try {
    const result = await fetchRequestCapture(trace, request.signal)
    if (!request.signal.aborted) {
      capture.value = result
      // A pending call gaining an outgoing capture must not silently change
      // the source of an editor the operator has already opened.
      chosenSource.value ??= result.outgoing ? 'outgoing' : 'original'
    }
  } catch (e) {
...
watch(() => props.traceId, () => { void loadCapture() }, { immediate: true })
watch(() => props.active, active => {
  if (active) void loadCapture()
  else { controller?.abort(); loading.value = false }
})
watch(() => props.refreshKey, () => {
  if (props.active && !capture.value?.outgoing && !loading.value) void loadCapture()
})
onUnmounted(() => controller?.abort())
defineExpose({ cancelOperation: () => replayWorkbench.value?.cancelOperation() })
</script>

<template>
  <div class="panel-scroll min-h-0 min-w-0 flex-1 space-y-4 p-3 sm:p-4" aria-label="原始与出站请求" tabindex="0">
    <p v-if="error" role="alert" class="text-sm text-destructive">{{ error }}<Button size="sm" variant="outline" class="ml-2 h-7 text-xs" :disabled="loading" @click="loadCapture">重新加载请求</Button></p>
    ...
    <template v-if="capture">
      ...
      <ContextComparison v-show="mode === 'context'" :trace-id="capture.trace_id" :logs="logs" :active="active && mode === 'context'" />
    </template>
    <ReplayWorkbench ref="replayWorkbench" v-show="mode === 'replay'" :trace-id="traceId" :source="source" :snapshot="snapshot" :refresh-key="replayRefreshKey" :active="active && mode === 'replay'" @select="trace => emit('select', trace)" @source="trace => emit('source', trace)" @operation="state => emit('operation', state)" />
  </div>
</template>
