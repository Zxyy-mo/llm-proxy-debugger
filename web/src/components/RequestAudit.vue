...
import type { ReplaySource, RequestCapture } from '@/lib/types'
...
const props = defineProps<{ traceId: string | null; refreshKey: number; replayRefreshKey: number }>()
const emit = defineEmits<{ select: [trace: string] }>()
const capture = ref<RequestCapture | null>(null)
const error = ref('')
const loading = ref(false)
const mode = ref<'compare' | 'curl' | 'replay'>('compare')
  ...
  <div class="panel-scroll min-h-0 min-w-0 flex-1 space-y-4 p-3 sm:p-4" aria-label="原始与出站请求" tabindex="0">
    ...
    <template v-if="capture">
      ...
      <div class="flex flex-wrap items-center gap-1 border-b pb-2" role="tablist" aria-label="请求操作">
        ...
        <div v-if="mode !== 'compare'" class="ml-auto flex items-center gap-2 text-xs">
      ...
      <CurlExportPanel v-else-if="mode === 'curl'" :trace-id="capture.trace_id" :source="source" :available="source === 'original' || Boolean(capture.outgoing)" />
